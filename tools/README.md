...
`gen_api.pl` emits the builder/accessor API over those nodes (spec-only
...
attribute (own + inherited, all defaulted) and returning the node via a
complete named aggregate — build bottom-up (children first, then pass their
cursors to the parent). `Diana.Accessors` has `Is_T`/`As_T` per type; read
attributes as `As_T (C).Attribute` (the record components are public, so one
typed view per type sidesteps the attribute-name collisions a per-attribute
getter scheme would hit). Re-run both after a spec change, alongside
`gen_nodes.pl`.
