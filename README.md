...
- **Values:** integer, real (with promotion), boolean, character literals
  ...
  over arrays and records (element-/component-wise) and lexicographic `<`/`<=`/
  `>`/`>=` over arrays, and array / record
...
- **Statements:** assignment, `if`, `while`, plain `loop`, range `for`, container `for ... of`
  ...
  **quantified expressions** (`for all` / `for some` over a range),
  **short-circuit** `and then` / `or else` (the Right operand is evaluated only
...
- **Generics:** generic-subprogram instantiation — `function F is new G (Actuals)`
  ...
  **incomplete** types (`type T;`, no operations, values only pass through), and
  formal **limited private** types (no `:=`/`=`, only supplied operations), and
