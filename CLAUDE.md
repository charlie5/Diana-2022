...
`Diana.Interpreter` is a first execution slice: it walks a tree through the accessors and evaluates it over the `Static_Value` model — integer / real / boolean / string literals, variable references, built-in operator calls (integer & real arithmetic with promotion, comparison over numbers/strings, string `&` concatenation, boolean ops), assignment, if-statements, while-loops, **range-based for-loops** (`reverse` and the Ada 2022 `when` iterator filter), **case statements** (value / `1 | 2` / `3 .. 4` range / `others` choices), statement sequences, `Put_Line`, **lexically-scoped local declarations** (block statements and declare-expressions elaborate object declarations into a fresh scope that shadows outer names and is discarded on exit), and **user-defined subprogram calls** with `in` / `out` / `in out` parameters (`out`/`in out` formals are copied back to the actual variable after the body; `Return_Statement` handling; recursion; **nested subprograms close over their enclosing activation** via a static link). Names resolve through a scope chain — scopes are kept in a vector and linked by enclosing-scope index; a subprogram-call scope parents to the scope where the subprogram was *declared* (its static link — the global scope for a top-level subprogram), and assignment to an undeclared name lands in the global scope. `interp_demo.adb` runs a `while`-loop summation (15), a recursive `Fact` (120), a block whose local `X` shadows a global (7 then 100), and a for-loop sum + for/case classifier (55, then 100/200/30/40/5), a `Swap` (`in out`) + `Get_Answer` (`out`) showing copy-back (2/1, then 42), and a real/string program (`Pi * Radius * Radius` = 12.5600, `"Hello, " & "DIANA" & "!"`, `Radius <= Pi` = True), and a nested `Inner` closing over `Outer`'s parameter (`Outer (5)` = 15), then shows the error path.

Still open: broadening the interpreter (access values/types, container/`for ... of` iterators, `exit`/`goto`). `diana_harness.adb` exercises the builder/accessor API and the merge / error-on-missing path; `interp_demo.adb` exercises execution.
