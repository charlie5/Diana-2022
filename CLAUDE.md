...
`Diana.Interpreter` is a first execution slice: it walks a tree through the accessors and evaluates it over the `Static_Value` model — integer/boolean literals, variable references, built-in operator calls (arithmetic + comparison), assignment, if-statements, while-loops, **range-based for-loops** (`reverse` and the Ada 2022 `when` iterator filter), **case statements** (value / `1 | 2` / `3 .. 4` range / `others` choices), statement sequences, `Put_Line`, **lexically-scoped local declarations** (block statements and declare-expressions elaborate object declarations into a fresh scope that shadows outer names and is discarded on exit), and **user-defined subprogram calls** (`Return_Statement` handling, recursion). Names resolve through a scope chain — scopes are kept in a vector and linked by enclosing-scope index; a subprogram-call scope parents to the global scope (top-level lexical), and assignment to an undeclared name lands in the global scope. `interp_demo.adb` runs a `while`-loop summation (15), a recursive `Fact` (120), a block whose local `X` shadows a global (7 then 100), and a for-loop sum + for/case classifier (55, then 100/200/30/40/5), then shows the error path.

Still open: broadening the interpreter (real/string/access values, `out`/`in out` parameter copy-back, nested subprograms / closures, container/`for ... of` iterators, `exit`/`goto`). `diana_harness.adb` exercises the builder/accessor API and the merge / error-on-missing path; `interp_demo.adb` exercises execution.
