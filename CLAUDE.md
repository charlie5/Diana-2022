...
`Diana.Interpreter` is a first execution slice: it walks a tree through the accessors and evaluates it over the `Static_Value` model — integer / real / boolean / string literals, **access values** (`new T'(x)` allocates onto a heap, `.all` dereferences, `null`, `X.all := v`, access equality/aliasing), **array / record aggregates** with composite value semantics (`(1, 2, 3)` / `(X => 1, Y => 2)`, indexing `A (I)`, selection `R.F`, component assignment, deep copy on `B := A`), variable references, built-in operator calls (integer & real arithmetic with promotion, comparison over numbers/strings, string `&` concatenation, boolean ops), assignment, if-statements, while-loops, **`for` loops** — range-based (`for I in L .. H`) and container (`for E of array`), both with `reverse` and the Ada 2022 `when` iterator filter, **case statements** (value / `1 | 2` / `3 .. 4` range / `others` choices), statement sequences, `exit` / `exit when` / named `exit`, `goto` and labelled statements (jumps within a sequence, propagating outward otherwise), `Put_Line`, **lexically-scoped local declarations** (block statements and declare-expressions elaborate object declarations into a fresh scope that shadows outer names and is discarded on exit), and **user-defined subprogram calls** with `in` / `out` / `in out` parameters (`out`/`in out` formals are copied back to the actual variable after the body; `Return_Statement` handling; recursion; **nested subprograms close over their enclosing activation** via a static link). Names resolve through a scope chain — scopes are kept in a vector and linked by enclosing-scope index; a subprogram-call scope parents to the scope where the subprogram was *declared* (its static link — the global scope for a top-level subprogram), and assignment to an undeclared name lands in the global scope. `interp_demo.adb` runs a `while`-loop summation (15), a recursive `Fact` (120), a block whose local `X` shadows a global (7 then 100), and a for-loop sum + for/case classifier (55, then 100/200/30/40/5), a `Swap` (`in out`) + `Get_Answer` (`out`) showing copy-back (2/1, then 42), and a real/string program (`Pi * Radius * Radius` = 12.5600, `"Hello, " & "DIANA" & "!"`, `Radius <= Pi` = True), a nested `Inner` closing over `Outer`'s parameter (`Outer (5)` = 15), an `exit when` + named `exit` from nested loops (15, then 203), and a `goto` loop (3), access values (`P := new Integer'(10); Q := P; P.all := 42` makes `Q.all` = 42 and `P = Q` True), and array/record aggregates (`Arr := (10,20,30); Cpy := Arr; Arr(2) := 99` leaves `Cpy(2)` = 20; `Rec := (X => 1, Y => 2); Rec.Y := 5` gives 1, 5), and `for ... of` iteration over an array (sum = 24; filtered `when E > 4` = 21), then shows the error paths (unbound variable, null dereference).

Still open: broadening the interpreter (`for ... of` over records, `case`/`for` over enumerations, more of the contract/aspect surface at runtime — e.g. `Pre`/`Post` checks). The `Static_Value` model (`harness/src/diana.ads`) is the interpreter's runtime value type; its `Access_Value` / `Array_Value` / `Record_Value` kinds are runtime-only handles into the interpreter's heap / array / record stores (access 0 = null; arrays and records get value semantics via a deep `Copy` on every store). `diana_harness.adb` exercises the builder/accessor API and the merge / error-on-missing path; `interp_demo.adb` exercises execution.
